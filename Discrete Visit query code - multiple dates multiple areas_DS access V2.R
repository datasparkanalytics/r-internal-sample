...
#This is a script template to call the Discreet Visit API for multiple dates and/or multiple locations
...
#
#   dsapi.dv.mdml(fromdate,todate,areas,output = list(uniqueVisit,totalVisit),hierarchy="discrete_visit_subzone",
#     interval = "PT1H", dimension = NULL,filters = NULL)
#
#   User needs to input three mandatory parameters:
...
#   There are three parameters with defaul value:
#     output - output of interest (uniqueVisit or totalVisit), default set as uniqueVisit. 
...
#     hierarchy - allows you to set the area hierarchy, default value is subzone
...
uniqueVisit <- list(metric = "unique_agents", type = "hyperUnique", describedAs = "footfall")
totalVisit <- list(metric = "total_records", type = "longSum", describedAs = "total_visits")

#A function to call the Discreet Visity API

dsapi.dv <- function(p_Date,area,output = list(uniqueVisit,totalVisit),hierarchy = "discrete_visit_subzone",
                        interval = "PT1H", dimension = NULL, filters = NULL){
...
dsapi.dv.mdml <- function(fromdate,todate,areas,output = list(uniqueVisit,totalVisit),hierarchy="discrete_visit_subzone",
                             interval = "PT1H", dimension = NULL, filters = NULL){
  ...
  o <- output #enter output of interest, uniqueVisit or totalVisit
  h <- hierarchy #location hierarchy config, can choose from subzone, planning area, planning region (e.g., "discrete_visit_subzone")
  p <- interval #enter reporting interval (format in PnDTnHnMnS), default at hourly interval (e.g., "PT1H)
  d <- dimension #enter reporting dimensions as a list (e.g. list("agent_gender","agent_race")), if not relevant, input "NULL"
  ...
  for (i in daterange) {
    for (j in area_list){
      data.df <- dsapi.dv(i,j,o,h,p,d,f)
